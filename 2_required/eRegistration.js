 ...
 function eRegistration(ticket, fullName, nowTime) {
    ...
    for (let flight in flights) {
        flights[flight]['tickets'].filter( currentTicket => {
            if (currentTicket.id == ticket) { ticketObject = currentTicket };
        })
    }
    if (!ticketObject) { throw new Error('Incorrect ticket number') };
    ...
    if (ticketObject.fullName !== fullName) {
        throw new Error('Another person\'s ticket');        
    }

    flight = flights[ticketObject.flight];

    if (nowTime < flight.registrationStarts) {
        throw new Error(
            'Registration starts in ' + 
            `${new Date(flight.registrationStarts)}. Please wait.` 
        );  
    } else if (nowTime > (flight.registartionEnds - 3600000)) {
        throw new Error(
...
eRegistration('BH118-B50', 'Ivanov I. I.', makeTime(10 ,1));